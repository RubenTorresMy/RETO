...
const designPoints = new Map<number, number>();

function getSizePrice(size: string | undefined) {
  const normalized = String(size || "").toLowerCase();
  if (["small", "pequena", "pequeno", "pequeña", "pequeño", "a4"].includes(normalized)) return 7;
  if (["medium", "mediana", "mediano", "a3"].includes(normalized)) return 10;
  if (["large", "grande", "a2"].includes(normalized)) return 15;
  if (["x-large", "personalizable", "custom", "a1"].includes(normalized)) return 20;
  return 10;
}
...
async function ensureFlagCatalog() {
  try {
    for (const product of flagProducts) {
      await pool.query(
        `INSERT INTO producto (id_producto, nombre, descripcion, precio_base, tipo_producto, publico, imagen)
...
const frontendPath = path.resolve(
  __dirname,
  path.basename(__dirname) === "dist" ? "../../frontend/src" : "../frontend/src"
);
...
app.get("/api/disenos", async (req: Request, res: Response) => {
  try {
...
function verifyToken(req: Request, res: Response, next: Function) {
  ...
  try {
    ...
    (req as any).userRole = decoded.rol ?? "user";
    next();
  } catch (error) {
...
function requireWorker(req: Request, res: Response, next: Function) {
...
app.post("/api/auth/register", async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      "INSERT INTO usuario (id_usuario, nombre_usuario, email, password, fecha_registro, rol) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id_usuario, nombre_usuario, email, fecha_registro, rol",
      [nextId, nombre_usuario, email, hashedPassword, new Date().toISOString().split('T')[0], "user"]
    );
      ...
      try {
        ...
        if (existingDemo.rows.length > 0) {
          ...
          const insertedDemo = await pool.query(
            "INSERT INTO usuario (id_usuario, nombre_usuario, email, password, fecha_registro, rol) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id_usuario",
            [demoUserId, "Admin Demo", email, passwordHash, new Date().toISOString().split("T")[0], "worker"]
          );
...
function createDesignCover(row: any, name: string) {
...
function formatSaleItem(item: any, index: number) {
...
function buildProductQuery(category: string) {
...
async function getCatalogItems(category: string) {
  const normalized = (category || '').toLowerCase();
...
async function getModernCatalogItems(category: string) {
  const normalized = (category || "").toLowerCase();
...
app.get("/api/profile", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const result = await pool.query(
      "SELECT id_usuario, nombre_usuario, email, fecha_registro, rol FROM usuario WHERE id_usuario = $1",
      [userId]
    );
...
app.post("/api/profile/password", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
...
app.get("/api/profile/orders", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const result = await pool.query(`
      SELECT
...
app.get("/api/profile/designs", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const result = await pool.query(`
      SELECT d.*, row_to_json(p) AS producto
...
app.get("/api/sale", async (req: Request, res: Response) => {
  try {
    ...
    const saleItems = items
      .filter((item: any) => Number(item.basePrice ?? 0) > 0)
...
app.get('/api/intranet/flags', verifyToken, requireWorker, (req: Request, res: Response) => {
  res.json({ success: true, data: intranetFlags });
});

app.delete('/api/intranet/flags/:id', verifyToken, requireWorker, (req: Request, res: Response) => {
...
app.get('/api/intranet/complaints', verifyToken, requireWorker, (req: Request, res: Response) => {
  res.json({ success: true, data: userComplaints });
});

app.post('/api/intranet/complaints', verifyToken, requireWorker, (req: Request, res: Response) => {
...
app.delete('/api/intranet/complaints/:id', verifyToken, requireWorker, (req: Request, res: Response) => {
  const id = Number(req.params.id);
...
app.get("/api/designs/popular", async (req: Request, res: Response) => {
  try {
    const result = await pool.query(`
      SELECT
        d.*,
...
app.delete("/api/designs/:id", verifyToken, requireWorker, async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const designId = parseInt(req.params.id as string, 10);
...
app.post("/api/designs/:id/like", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const designId = parseInt(req.params.id as string, 10);
    ...
    const likeResult = await pool.query(
      "INSERT INTO diseno_like (id_usuario, id_diseno) VALUES ($1, $2) ON CONFLICT (id_usuario, id_diseno) DO NOTHING RETURNING id_diseno",
      [userId, designId]
    );
...
app.delete("/api/designs/:id/like", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const designId = parseInt(req.params.id as string, 10);
...
app.get("/api/profile/favorites", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const result = await pool.query(`
...
app.post("/api/profile/favorites/:designId", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const designId = parseInt(req.params.designId as string, 10);
    const design = await pool.query("SELECT id_diseno, id_usuario FROM diseno WHERE id_diseno = $1", [designId]);
    ...
    const favoriteResult = await pool.query(
      "INSERT INTO diseno_favorito (id_usuario, id_diseno) VALUES ($1, $2) ON CONFLICT (id_usuario, id_diseno) DO NOTHING RETURNING id_diseno",
      [userId, designId]
    );
...
app.get("/api/profile/rewards", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const result = await pool.query(
      "SELECT id_diseno FROM diseno WHERE id_usuario = $1",
      [userId]
    );

    const points = result.rows.reduce((total, row) => (
      total + (designPoints.get(Number(row.id_diseno)) || 0)
    ), 0);
...
app.delete("/api/profile/favorites/:designId", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const designId = parseInt(req.params.designId as string, 10);
...
app.post("/api/designs", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const {
      name,
...
app.post("/api/cart", verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    const existingResult = await pool.query(
      ...
      [userId, id_producto, id_diseno, tamano]
    );
    ...
    if (existingResult.rows.length > 0) {
      ...
      const updateResult = await pool.query(
        `UPDATE carrito SET cantidad = $1 WHERE id_carrito = $2 RETURNING *`,
        [newQuantity, cartId]
      );
    ...
    // Agregar nuevo item
    const insertResult = await pool.query(
      `INSERT INTO carrito (id_usuario, id_producto, id_diseno, cantidad, tamano, precio_unitario)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [userId, id_producto || null, id_diseno || null, cantidad, tamano || null, precio_unitario]
    );
...
app.post("/api/designs/:id/reward-use", verifyToken, async (req: Request, res: Response) => {
  const designId = parseInt(req.params.id as string, 10);
  const points = (designPoints.get(designId) || 0) + 10;
  designPoints.set(designId, points);
  res.json({
...
app.put("/api/cart/:cartId", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
...
app.delete("/api/cart/:cartId", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const cartId = parseInt(req.params.cartId as string, 10);
...
app.delete("/api/cart", verifyToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
